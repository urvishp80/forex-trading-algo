...
from Automated_Signal_generator import (signalStrategy, Signal,
                                        Run)
from Strategy_ml.forex_inference import forex_inference
...
class streamSignal(ttk.Frame):
    ...
    def liveSignal(self):
        ...
        for st in self.pairs:
            ...
            data = pd.read_csv(os.path.join(self.path['mainPath'],
                                            f"Strategy_ml\\data\\current_price_prediction\\predicted_{st}" + ".csv"))
        ...
        if not os.path.exists(os.path.join(path['mainPath'], path['signals'] + '\\signals.csv')):
            ...
            signal.to_csv(os.path.join(path['mainPath'], path['signals'] + '\\signals.csv'), index=False)
            # --Return telegram
            telegramBot(self.path).tgsignal(signal)
        else:
            signal = pd.read_csv(os.path.join(path['mainPath'], path['signals'] + '\\signals.csv'), index_col=False)

            if signal['pair'].values[0] != openPositions[0][0]:
                os.remove(os.path.join(path['mainPath'], path['signals'] + '\\signals.csv'))
                signal = pd.DataFrame(openPositions, columns=columns)
                signal.to_csv(os.path.join(path['mainPath'], path['signals'] + '\\signals.csv'), index=False)
                # --Return telegram
                telegramBot(self.path).tgsignal(signal)
    ...
    def signalGUI(self):
        ...
        time.sleep(2)
        for i in self.pairs:
            test_path = os.path.join(self.path['mainPath'],
            ...
            if "close" in test_path_df.columns:
                ...
                test_path_df = pd.read_csv(test_path, names=["timestamp", "completed", "open","high","low","close","volume"])
            # test_path_df = pd.read_csv(test_path, names=["timestamp", "completed", "open","high","low","close","volume"])
            print("length of df",len(test_path_df))
            test_path_df.to_csv(test_path, index=False)

            forex_inference(i, test_path)
        # throw signal
    ...
    def plotReturns(self):
        ...
        for stockPair in pairs:
            data = pd.read_csv(os.path.join(path['mainPath'],
                                        f"Strategy_ml\\data\\current_price_prediction\\predicted_{stockPair}" + ".csv"))
...
class visu(ttk.Frame):
    ...
    def plots(self):
        ...
        grabtimeframe = str(self.timeOption.get())
        data = pd.read_csv(os.path.join(path['mainPath'],
                                        f"Strategy_ml\\data\\current_price_prediction\\predicted_{grabpair}" + ".csv"))
        data['timestamp'] = pd.to_datetime(data['timestamp'])
        ...
        ax2.plot(np.arange(len(data)), data.Close, lw=1., color='#e64af0')
...
if __name__ == '__main__':
    ...
    import time
    import datetime
    print(str(datetime.datetime.utcnow().isoformat('T')[:-7] + 'Z'))
